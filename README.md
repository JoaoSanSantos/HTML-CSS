...
Para elementos de cores é usado o input:color
onde vai aparecer um bloco com as 63 milhoes de cores.
...
input:range é usado para definir Níveis, por exemplo Nível de satisfação que vai de 0 a 100
...
Podemos alterar esse de 0 a 100 para qualquer valor adicionando o parametro min e max.
Mas não confunda com minlength e maxlength, são coisas diferentes.
A barra sempre começa em um valor padrão no meio, para mudar isso adiciona-se value. Dessa forma fica tudo como descrito abaixo:

    
            <label for="inivel">Nivel de Satisfação</label>
            <input type="range" name="nivel" id="inivel" min="0" max="10" value="0">

O input:file é para enviar arquivos como documentos e fotos. Escreve-se dessa forma:
...
Porém na hora do envio ocorre um problema. 
Quando utilizar esse tipo de controle a tag form não pode estar no method GET, precisa ser o method POST para que as imagens sejam enviadas.

Aula 10 - Select, datalist e textarea

Esses elementos já não são mais usados na tag input.
A tag Select é pra selecionar algo, no exemplo abaixo está sendo utilizado Estados.

Dentro de select adiciona-se a tag option para as opções a serem selecionadas.
...
É possível agrupar por regiões cada tag option, basta envelopar as tags desejadas com optgroup e dentro dele adicionar Label "região xyz", lembrando que esse label não é elemento e sim atributo.
...
Para deixar algum estado já selecionado, dentro de option após value adiciona-se selected

    <option value="MT" selected >Mato Grosso</option>

Ou você pode deixar uma tag option só pra selected e o nome, para que apareça somente o que você escreveu, como descrito abaixo:

    <option value="" selected>---Escolha---</option>

Porém dessa forma, o usuário só irá escolher de acordo com as opções que você previamente forneceu a ele. Caso essa opção não exista, existe uma outra opção que são os datalist.
...
O Datalist também será usado dentro dele a tag option, no caso acima foi para opções.
Mas não basta só adicionar options que não irá aparecer nada, antes disso precisa dar um id para o datalist e colocar esse id, atraves do atributo list, dentro do input.
O datalist, além de poder escolher as opções, você também pode escrever, coisa que não da pra fazer em select.
É possível organizar no W3C o optgroup em datalist, mas visualmente não esta funcionando ainda.


E o ultimo elemento é textearea, que nada mais é que uma caixa de texto. Para usa-lo basta escrever como esta abaixo :
...
Não existe limite, porém o formulário também precisa ser method POST.

Aula 11 - Elemento Output em formulários
